...
CREATE TABLE `__new_transactions` (
	...
	CONSTRAINT "status_check" CHECK("__new_transactions"."status" IN ('pending_payment', 'pending_verification', 'verified', 'rejected')),
	CONSTRAINT "email_check" CHECK("__new_transactions"."customer_email" LIKE '%@%.%')
);
--> statement-breakpoint
INSERT INTO `__new_transactions`("id", "event_id", "customer_name", "customer_email", "customer_whatsapp", "ticket_category", "ticket_quantity", "total_amount", "payment_proof_url", "status", "created_at") SELECT "id", "event_id", "customer_name", "customer_email", "customer_whatsapp", "ticket_category", "ticket_quantity", "total_amount", "payment_proof_url", "status", "created_at" FROM `transactions`;--> statement-breakpoint
